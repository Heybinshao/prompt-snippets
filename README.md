# Prompt Snippets — Hermes 桌面端自定义提示词片段 ![版本](https://img.shields.io/badge/版本-v1.3.2-blue)
...
| 输入框「+」菜单 / ⌘K「打开我的片段」 | 管理列表：新增 / 编辑 / 删除 / 拖拽排序（⠿） | 维护片段库 |
...
3. **管理**：「+」菜单 →「我的片段」，每行右侧依次是 编辑（✎）/ 删除（✕）/ 拖动块（⠿）；**按住 ⠿ 上下拖动即可排序**（整行浮起、其他行实时让位）；点片段名本身 = 直接插入
